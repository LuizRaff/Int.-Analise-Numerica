...
void teste_derivada (double (*f) (double), double (*fl) (double), double x)
{
  for (int i = 1; i < 13; ++i) {
    double h = pow(10, -i);
    double d = derivada(f, x, h);          // calculo da derivada numérica
    double a = fl(x);                       // valor analítico
    printf("%e  %.16g  %.16g\n", h, d, fabs(d - a));
  }
}
...
{
  double s16 = simpson(f, a, b, 16);       // Simpson composto com 16 subintervalos
  double s32 = simpson(f, a, b, 32);       // Simpson composto com 32 subintervalos
  printf("%.16g  %.16g\n", s16, s32);
}
...
void teste_adaptativo (double (*f) (double), double a, double b, double d)
{
  for (int i = 1; i < 13; ++i) {
    double tol = pow(10, -i);
    double s = simpsonadaptativo(f, a, b, tol);  // Simpson adaptativo com tolerância tol
    printf("%e %.16g %e\n", tol, s, fabs(d - s));
  }
}
